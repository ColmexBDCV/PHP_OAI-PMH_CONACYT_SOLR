...
class OAI2Server {
        ...
        if (empty($this->errors)) {
            try {
                ...
                if ($records_count - $deliveredRecords > $maxItems) {
                    ...
                    $restoken = $this->createResumptionToken($deliveredRecords, $metadataPrefix);
    ...
    private function add_metadata($cur_record, $record) {
        ...
        foreach ($record['metadata']['fields'] as $name => $value) {
            ...
            $this->response->addChild($schema_node, $name, $value, $record['metadata']['attributes']);
        }
    }

    private function createResumptionToken($delivered_records, $metadataPrefix) {
