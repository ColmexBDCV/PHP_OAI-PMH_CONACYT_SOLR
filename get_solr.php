    ...
    foreach ($reg as $key => $value) {
        ...
        if ($key=="id"){
            ...
            foreach($value as $v)
            {
                ...
                if (is_int(strpos($v, 'info:eu-repo/semantics/')) === false) {
                    $v = "info:eu-repo/semantics/".$v;
                }
                
                if($v == "info:eu-repo/semantics/masterDegreeWork") {
                
                    $v = "info:eu-repo/semantics/masterThesis";
                }
                $value[$i] = $v;
            ...
            if ($key == "visibility") {
                $value = array("info:eu-repo/semantics/".add_access_rights($value), "http://creativecommons.org/licenses/by-nc-nd/4.0");
...
function get_records($start = 0, $rows=20)
{
    ...
    $select = array(
       ...
       'filterquery' => array(
           "work_type" => array(
                "query" => 'resource_type_tesim: ("Trabajo de investigación de Maestría" OR "Tesis de Doctorado" OR "Tesis de Maestría" OR "Artículo en revista COLMEX" OR "Reseña en revista COLMEX" OR "Libro" OR "Capítulo de Libro")'
...
//krumo(get_records($start = 0, $rows=2000));

// krumo(get_record('1z40kt475'));
