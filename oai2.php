...
$identifyResponse["repositoryName"] = 'REPOSITORIO DE EL COLEGIO DE MÉXICO';
$identifyResponse["baseURL"] = 'http://repositorio.colmex.mx/';
$identifyResponse["protocolVersion"] = '2.0';
$identifyResponse['adminEmail'] = 'rcuellar@colmex.mx';
$identifyResponse["earliestDatestamp"] = '2017-09-21T12:00:00';
...
$identifyResponse["granularity"] = 'YYYY-MM-DDThh:mm:ss';


function record_builder($reg) {

    $record = array('identifier' => $reg["identifier"],
    ...
    $record['metadata']['fields'] = $reg["fields"];
    $record['metadata']['attributes'] = $reg["attributes"];
...
$oai2 = new OAI2Server($uri, $args, $identifyResponse,
    array(
        ...
        function($metadataPrefix, $from = '', $until = '', $set = '', $count = false, $deliveredRecords = 0, $maxItems = 0) {
            $d = get_records($deliveredRecords, $maxItems);     
            if ($count) {
                return $d[1];
            }
            if ($set != '') {
            ...
            $records = [];
            foreach ($data as $r) {
                array_push($records, record_builder($r));
            }
        ...
        function($identifier, $metadataPrefix) {
            $record =  record_builder(get_record($identifier));
            if (is_null($record)) {
                throw new OAI2Exception('idDoesNotExist');
            }
            return $record;
