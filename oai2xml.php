...
class OAI2XMLResponse {
    ...
    function __construct($uri, $verb, $request_args) {
        ...
        $this->doc->appendChild($this->doc->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="stylesheets/oai2.xsl"'));
    ...
    function addChild($mom_node,$name, $value='', $attr= false) {
        if(is_array($value)){
            foreach($value as $v) {
                $added_node = $this->doc->createElement($name,$v);
                if($name == "dc:creator" || $name == "dc:contributor"){
                    $add_attr = $this->doc->createAttribute('id');
                    $add_attr->value = $attr[$v];
                    $added_node->appendChild($add_attr);
                }
